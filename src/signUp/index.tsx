...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  className={`btn btn-primary py-3 px-4 rounded-xl font-semibold cursor-pointer transition-all duration-300 
    ${isLoading ? 'opacity-50 cursor-not-allowed' : 'bg-gradient-to-br from-[#7c5cff] to-[#00d1ff] text-[#0b0d10] shadow-lg'}`}
>
  {isLoading ? 'Signing Up...' : 'Sign Up'}
      ...
      <p className="text-center text-text-dim text-sm mt-6">
        ...
        <a href="/login" className="text-primary hover:text-accent transition-colors">
          Sign in
        </a>
      </p>
    </div>
